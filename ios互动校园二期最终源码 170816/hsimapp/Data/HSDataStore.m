...
static HSDataStore *sharedInstance;

static NSString *const DEFAULT_DB_NAME = @"HSDataStore.sqlite";
...
+(void)initialize
{
    ...
    if (!initialized)
    {
        ...
        sharedInstance = [[HSDataStore alloc] init];
    }
}
...
- (id)initDBWithName:(NSString *)dbName
{
    ...
    if (self)
    {
        ...
        if([self isTableExists:@"default_data_table"] == NO)
        {
            [self createTableWithName:@"default_data_table"];
        }
    }
    return self;
}
...
-(void)clearCache
{
    [self deleteObjectById:@"firstLaunch"];
}

- (void)createTableWithName:(NSString *)tableName
{
    if ([HSDataStore checkTableName:tableName] == NO)
    {
        return;
    }
...
{
    [self putObject:object withId:objectId intoTable:@"default_data_table"];
}

- (id)getObjectById:(NSString *)objectId
{
    return  [self getObjectById:objectId fromTable:@"default_data_table"];
}

-(void)deleteObjectById:(NSString *)objectId
{
    [self deleteObjectById:objectId fromTable:@"default_data_table"];
}
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    HSKeyValueItem * item = [self getHSKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (HSKeyValueItem *)getHSKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    if (json) {
        ...
        HSKeyValueItem * item = [[HSKeyValueItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (HSKeyValueItem * item in result) {
        error = nil;
