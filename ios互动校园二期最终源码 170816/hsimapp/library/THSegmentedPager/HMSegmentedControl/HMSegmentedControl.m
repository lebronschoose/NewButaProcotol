...
{
    NSMutableDictionary *dictBadges;
}
...
- (void)setBadge:(int)count ofIndex:(int)index
{
    [dictBadges setObject:[NSNumber numberWithInt:count] forKey:[NSString stringWithFormat:@"%d", index]];
}

- (void)commonInit
{
    dictBadges = [[NSMutableDictionary alloc] init];
        ...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop)
         {
            ...
            if(YES)
            {
                NSNumber *count = [dictBadges objectForKey:[NSString stringWithFormat:@"%lu", (unsigned long)idx]];
                if(count != nil && count.intValue > 0)
                {
                    CATextLayer *badgeLayer = [CATextLayer layer];
                    badgeLayer.frame = CGRectMake((self.segmentWidth * idx) + stringWidth + (self.segmentWidth - stringWidth) / 2.0 - 2, rect.origin.y - 4, 14, 14);
                    badgeLayer.fontSize = 10;
                    badgeLayer.alignmentMode = kCAAlignmentCenter;
                    badgeLayer.string = [NSString stringWithFormat:@"%@", count];
                    badgeLayer.truncationMode = kCATruncationEnd;
                    ...
                    [badgeLayer setMasksToBounds:YES]; [badgeLayer setCornerRadius:7.0];
