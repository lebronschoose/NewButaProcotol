...
- (void)viewDidLoad
{
    ...
    if(self.transObj != nil && ([self.transObj isKindOfClass:[NSDictionary class]] || [self.transObj isKindOfClass:[NSMutableDictionary class]]))
    {
        NSDictionary *dict = self.transObj;
        NSLog(@"transObjdict is %@",dict);
        NSString * titleString = [dict objectForKey:@"title"];
        self.navigationItem.title = titleString;
        NSString *url = [dict objectForKey:@"url"];
        if ([titleString isEqualToString:@"校园商城"]) {
            url = [NSString stringWithFormat:@"%@/mall/index.php?checkcode=%@", [HSAppData getDomain], [HSAppData getCheckCode]];
        }else if([titleString isEqualToString:@"报名系统"])
        {
            self.navigationItem.title = [dict objectForKey:@"schooltitle"];
            url =   [NSString stringWithFormat:@"%@/Actives/index/checkcode/%@.html",[HSAppData getDomain],[HSAppData getCheckCode]];
        }
