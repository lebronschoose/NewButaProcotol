        ...
        if ([self.cacheText isEqualToString:self.textView.text]) {
            ...
            if (self.textView.text.length==0)
            {
                ...
                if (MHStringIsNotEmpty(self.cacheText)) {
                    // 存@""值
                    [[MHTopicManager sharedManager].replyDictionary setValue:@"" forKey:self.model.cid];
                }
            }else{
                [[MHTopicManager sharedManager].replyDictionary setValue:self.textView.text forKey:self.model.cid];
            }
        }
    }else{
        // 解析数据
        if (self.model.cid) {
            [[MHTopicManager sharedManager].replyDictionary removeObjectForKey:self.model.cid];
        }
    }
...
{
    _model = model;
//    self.commentLabel.text = model.aname;
    self.commentLabel.text  = _Master.mySelf.nickName;
    self.avatarView.image = [UIImage imageNamed:@"mh_defaultAvatar@2x.png"];
    self.textView.placeholderText = [NSString stringWithFormat:@"回复%@",model.qname];
    // 设置text
    // 获取缓存text
    self.cacheText = [[MHTopicManager sharedManager].replyDictionary objectForKey:model.cid];
    // 缓存字体
...
- (void)_setupBottomToolBar
{
    ...
    // 评论  MHFont(MHPxConvertPt(11.0f), NO)
    ...
    // textViewMHFont(MHPxConvertPt(14.0f), NO)
    ...
    // words MHFont(MHPxConvertPt(9.0f), NO)
    YYLabel *words = [[YYLabel alloc] init];
    words.textAlignment = NSTextAlignmentLeft;
    
    NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:@"0/0字"];
    attributedText.yy_font = [UIFont systemFontOfSize:9];
...
- (void)_bottomToolBarWillChangeHeight:(CGFloat)toHeight
{
    ...
    // 不允许遮盖住 视频播放
    CGFloat maxHeight = ScreenHeight - MHTopicCommentToolBarMinTopInset - self.keyboardHeight;
...
{
    NSString *text = [NSString stringWithFormat:@"%zd/%zd",textView.attributedText.length , _wordlength];
    UIFont *font = MHFont(MHPxConvertPt(9.0f), NO);
    UIColor *color = textView.attributedText.length<=_wordlength ? MHGlobalGrayTextColor : [UIColor redColor];
...
- (void) _send
{
    ...
    if (self.textView.attributedText.length >_wordlength) {
